...
class BMDialView: UIView, UITextFieldDelegate {
    ...
    var CallButtonColor = UIColor.custom_lightGreen
    var TextColor = UIColor.custom_black
    ...
    private func setupUI() {
        ...
        textField?.frame = CGRect.init(x: gap/2,
                                       y: (frame.size.height - requiredKeyPadHeight - 100 )/2,
                                       width: self.frame.size.width - gap,
                                       height: 100)
...
//        let yGap = xGap
        ...
        callBtn.setImage(UIImage.init(named: "Phone Filled"), for: UIControl.State.normal)
